 ...
 * created: 29.11.2009 11:57:11
 */

/**
 * Dealing with data. Can pluralize, singularize etc
 *
 * @version 1.0
 *
 * @author Alexandr Viniychuk <alexandr@viniychuk.com>
...
class slInflector {
	...
	static public function slugify($text) {
		...
		$text = preg_replace('~[^\\pL\d\s]+~', '_', $text);
    ...
    static public function parseDateUkr($date) {
        $date = trim($date);
        ...
        foreach($months as $i=>$month) {
            if (mb_strpos($date, $month) !== false) {
                $res['m'] = $i+1;
            }
        }
        $parts = explode(' ', $date);
        if (count($parts) > 2) {
            if (mb_strlen($parts[0]) > mb_strlen($parts[2])) {
                $res['d'] = $parts[2];
                $res['y'] = $parts[0];
            } else {
                $res['y'] = $parts[2];
